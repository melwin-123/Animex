...
import {
  ...
  SkipBack, SkipForward, ChevronLeft, Download, List, CheckCircle2
} from 'lucide-react';
...
export default function WatchPage() {
  ...
  const { updateWatchProgress, startDownload, preferences } = useAnime();
  ...
  const [quality, setQuality] = useState(preferences.quality || '1080p');
  const [showSettings, setShowSettings] = useState(false);
  const [playbackSpeed, setPlaybackSpeed] = useState(1);
  const [hasCompletedToast, setHasCompletedToast] = useState(false);
  ...
  useEffect(() => {
    setQuality(preferences.quality || '1080p');
  }, [preferences.quality]);

  useEffect(() => {
    setProgress(0);
    setIsPlaying(false);
    setHasCompletedToast(false);
    if (anime && currentEp) updateWatchProgress(anime, currentEp, 0);
  }, [animeId, episodeId]);
  ...
  useEffect(() => {
    if (isPlaying) {
      progressTimerRef.current = setInterval(() => {
        setProgress((p) => {
          const next = p + (100 / duration) * playbackSpeed;
  ...
  useEffect(() => {
    ...
    updateWatchProgress(anime, currentEp, progress);

    if (progress >= 95 && !hasCompletedToast) {
      setHasCompletedToast(true);
      addToast(`Episode ${currentEp.number} completed!`, 'success');
      if (preferences.autoPlay && episodes[epIndex + 1]) {
        setTimeout(() => navigate(`/watch/${animeId}/${episodes[epIndex + 1].id}`), 1200);
      }
    }
  }, [progress]);
  ...
  const handleDownload = () => {
    startDownload(anime, currentEp);
    addToast(`Downloading Ep ${currentEp.number}...`, 'info');
  };

  const goEp = (dir) => {
    const next = episodes[epIndex + dir];
    if (next) navigate(`/watch/${animeId}/${next.id}`);
  };
      ...
      <div className="watch-page page-enter">
        <div className="watch-layout">
          <div className="player-container">
            <div className={`video-player ${showControls ? 'controls-visible' : ''}`} onMouseMove={handleMouseMove} onMouseLeave={() => isPlaying && setShowControls(false)} onClick={() => setIsPlaying(!isPlaying)}>
              <div className="fake-video">
                <img src={`https://images.unsplash.com/photo-${1578632767115 + currentEp.id * 997}?w=1280&h=720&fit=crop`} alt="anime scene" className="fake-video-img" />
              ...
              <div className="player-controls" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="controls-row">
                  <div className="controls-left">
                    <button className="ctrl-btn" onClick={() => goEp(-1)} disabled={epIndex === 0}><SkipBack size={18} /></button>
                    <button className="ctrl-btn play-pause-btn" onClick={() => setIsPlaying(!isPlaying)}>
                      {isPlaying ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" />}
                    </button>
                    <button className="ctrl-btn" onClick={() => goEp(1)} disabled={epIndex === episodes.length - 1}><SkipForward size={18} /></button>

                    <div className="volume-ctrl">
                      <button className="ctrl-btn" onClick={() => setIsMuted(!isMuted)}>{isMuted || volume === 0 ? <VolumeX size={18} /> : <Volume2 size={18} />}</button>
                      <input type="range" min="0" max="100" value={isMuted ? 0 : volume} className="volume-slider" onChange={(e) => { setVolume(Number(e.target.value)); setIsMuted(false); }} />
                    </div>
                  ...
                  <div className="controls-right">
                    <button className="ctrl-btn quality-btn" onClick={() => setShowSettings(!showSettings)}>{quality}</button>
                    <button className="ctrl-btn" onClick={() => setShowEpList(!showEpList)}><List size={18} /></button>
                    <button className="ctrl-btn" onClick={handleDownload}><Download size={18} /></button>
                    <button className="ctrl-btn" onClick={() => setIsFullscreen(!isFullscreen)}>{isFullscreen ? <Minimize size={18} /> : <Maximize size={18} />}</button>
                  </div>
                </div>
              </div>
            ...
            <div className="player-info">
              <div>
                ...
                <p className="player-ep-sub">Subtitles {preferences.subtitles ? 'on' : 'off'} · Auto-play {preferences.autoPlay ? 'enabled' : 'disabled'}</p>
              </div>
              <div className="player-info-actions">
                <button className="btn btn-ghost" onClick={handleDownload}><Download size={16} /> Download</button>
                {progress >= 95 && <button className="btn btn-primary"><CheckCircle2 size={16} /> Completed</button>}
          ...
          <div className={`ep-sidebar ${showEpList ? 'open' : ''}`}>
            ...
            <div className="ep-sidebar-list">
              {episodes.slice(0, 48).map((ep) => (
                <div key={ep.id} className={`ep-sidebar-item ${ep.id === currentEp.id ? 'active' : ''}`} onClick={() => navigate(`/watch/${animeId}/${ep.id}`)}>
